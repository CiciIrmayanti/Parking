              ...
              <div class="col-lg-12 col-sm-12 col-12">
                <div class="card">
                  ...
                  <div class="row">
                        <div >
                        <a href="
                          <?php echo site_url('Voucher/tambah') ?>"   class='btn bg-gradient-primary ml-1 mr-1 mb-1' >
                  ...
                  <div class="table-responsive">
                        <table id="DataMember" class="table zero-configuration">
                          <thead>
                            <tr>
                              <th>No</th>
                              <th>ID Voucher</th>
...
var dataTable = $('#DataMember').DataTable( {
"processing":true,
"serverSide": true,
"stateSave" : false,
"AutoWidth": true,
...
"ajax":{
url :"<?php echo site_url('Voucher/get_data_voucher'); ?>",
type: "POST",
error: function(){
$(".my-grid-error").html("");
...
$('#DataMember').DataTable().ajax.reload( null, false );
}
});
}
...
$(document).on('click', '#TambahVoucher, #edit, #detail', function(e){
e.preventDefault();

if($(this).attr('id') == 'TambahVoucher')
{
...
if($(this).attr('id') == 'DetailVoucher')
{
$('.modal-dialog').removeClass('modal-sm');
$('.modal-dialog').addClass('modal-lg');
$('#ModalHeader').html('Detail');
}
if($(this).attr('id') == 'EditVoucher')
{
$('.modal-dialog').removeClass('modal-sm');
...
$(document).on('keyup', '.id_Voucher', function(){
$(this).parent().find('span').html("");

var Kode = $(this).val();
var Indexnya = $(this).parent().parent().index();
var Pass = 0;
$('#TabelTambahVoucher tbody tr').each(function(){
...
$('#SimpanTambahVoucher').addClass('disabled');
}
else
{
$(this).parent().find('span').html('');
$('#SimpanTambahVoucher').removeClass('disabled');
...
if(json.status == 0){
 $('#TabelTambahVoucher tbody tr:eq('+Indexnya+') td:nth-child(2)').find('span').html(json.pesan);
 $('#SimpanTambahVoucher').addClass('disabled');
}
if(json.status == 1){
 $('#SimpanTambahVoucher').removeClass('disabled');
